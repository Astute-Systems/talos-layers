...
**Option 1 — shell env file** (works for all terminals, including running BMS directly):

```bash
# ~/.config/gva/api-keys.env  (chmod 600)
export AISTREAM_TOKEN=<your AISStream.io key>
export BODS_API_KEY=<your UK BODS key>
...
Add to `~/.bashrc`:
```bash
[ -f ~/.config/gva/api-keys.env ] && source ~/.config/gva/api-keys.env
```

**Option 2 — VS Code only**: add to `terminal.integrated.env.linux` in `.vscode/settings.json`:
